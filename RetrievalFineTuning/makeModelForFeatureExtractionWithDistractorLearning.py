...
if __name__ == "__main__":
    ...
    OutputModelName = "VGG16WithDistractorModel2"
    modelName = "./VGG16WithDistractorModel"
    fromModel = True
    dir = "../CombinedDataset"
    image_size = (224, 224)
    batch_size = 32
    ...
    lastNotTrainableLayer = "block3_pool"
    # model = load_model(modelName)
    check = True
    ...
    model.summary()
    '''
    x = model.get_layer("batch_normalization").output
    x = Dense(384, activation=tf.keras.layers.LeakyReLU(alpha=0.3))(x)
    x = Dropout(0.5)(x)
    ...
    batch_size = 32
    log_dir = "logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + OutputModelName
